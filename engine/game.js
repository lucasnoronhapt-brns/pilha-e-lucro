...
export const pesoAtual = st => st.stack.reduce((a,s)=>a+ING[s].peso,0);

const draw = st => POOL[Math.floor(st.rnd()*POOL.length)];
...
export function calc(st, stack=st.stack){
  ...
  stack.forEach((k,i)=>{
    ...
    chips += c;
    if(k==='queijo'){
      const sobreProteina = i>0 && ING[stack[i-1]].cat==='proteina';
  ...
  st.receitas.forEach(r=>{
    let hit=false;
    if(r.seq){
      for(let i=0;i+r.seq.length<=stack.length;i++){
        if(r.seq.every((s,j)=>stack[i+j]===s)){ hit=true; break; }
      }
    } else if(r.count){ hit = stack.filter(s=>s===r.count[0]).length>=r.count[1]; }
    else if(r.countCat){ hit = stack.filter(s=>ING[s].cat===r.countCat[0]).length>=r.countCat[1]; }
    if(hit){
...
export function colocar(st, i){
  ...
  st.log.push({t:'colocar', i});
  st.hand.splice(i,1); st.stack.push(k);
  refill(st);
