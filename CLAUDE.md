...
- **Loja entre rondas**: 2 staff + 2 receitas aleatórias, renderizadas com molduras de carta; dinheiro = 4 + serviços não usados + trocas não usadas. **Vender** (jul 2026): na loja podes vender staff/receitas por metade do preço (floor; Cheeseburger inicial vende por 0€) para libertar slots — ação `vender` no log, fila "Vender:" com confirmação em popup. **Equipamento** (M4, jul 2026): +1 oferta de item permanente (não sellável, não conta para os slots 3/3) sempre que sobrar algum por comprar
...
8. **O nome também não vem do cliente** (ago 2026): submeter exige sessão iniciada e o servidor vai buscar o nome ao perfil (`pl_perfis`), onde já passou pelo `shared/nome.js`. O `pl-submit-run` ignora por completo qualquer `nome` no corpo do pedido — o campo de texto livre deixou de existir no caminho da submissão
...
- ~~**M2.5 — Contas e identidade**~~ ✅ concluído (ago 2026): login com **Google** (OAuth). Escolhido por não enviar emails — o serviço interno da Supabase só permite **2 emails/hora** no plano free, e a confirmação de email está desligada no projeto (partilhado com o bolão, cujos registos não podem ser afetados). Tabela `pl_perfis` (nome único ignorando maiúsculas, ligada a `auth.users`), Edge Function `pl-perfil` (o único sítio onde um nome entra no sistema), coluna `pl_scores.user_id`. O jogo continua a **jogar-se sem conta**: a sessão só é pedida no ecrã de fim, ao submeter, e a run sobrevive ao redirect do Google via `sessionStorage`. Fluxo OAuth em `client/auth.js`, só com `fetch` (sem dependências). Credenciais em `segredos.local.md`, fora do git (`*.local.md` no `.gitignore`).
- ~~**M3 — Perecibilidade**~~ ✅ concluído (jul 2026, ver Mecânicas). Sprites de estado do tomate feitos em ago 2026; faltam os restantes ingredientes (usam filtro CSS). Contaminação fica como boss futuro ("Frigorífico Avariado").
- **M4 — Equipamentos** (moldura industrial, tipo vouchers) — 1ª fatia concluída (jul 2026): item permanente, comprado uma vez na loja, não ocupa slots de staff/receitas e não se vende. Aparece como 1 oferta extra na loja (moldura `card_equip.png`) e como secção própria na Coleção do Balcão. 3 itens: Freezer Novo (+1 slot staff), Estante Extra (+1 slot receitas), Fornecedor Direto (−1€ em todas as ofertas, mín. 1€) — `EQUIPAMENTOS` em `engine/data.js`, estado `st.equip` + `staffMax`/`receitaMax`/`precoOferta` em `engine/game.js`. Pendente: "chapa dupla" (depende do grill do M5).
